...
class Populer_Listings extends \WP_Widget
{
	...
	public function __construct()
	{
		...
		parent::__construct(
			'adqs_populer_listings',  // Base ID
			esc_html__('AD : Populer Listings', 'adirectory'), // Name
			array('description' => esc_html__('You can display populer listings by Select options.', 'adirectory')) // Args
		);
	}
		...
		<p>
			...
			<select id="<?php echo esc_attr($this->get_field_id('order_by')); ?>" name="<?php echo esc_attr($this->get_field_name('order_by')); ?>">
				...
				$orderByItems = apply_filters(
					'adqs_populerlistings_order_by',
					array(
						'views-desc'   => esc_html__('Popular (by views)', 'adirectory'),
						'rating-desc'  => esc_html__('Popular (by ratings)', 'adirectory'),
						'review-count' => esc_html__('Popular (by review count)', 'adirectory'),
					)
				);
