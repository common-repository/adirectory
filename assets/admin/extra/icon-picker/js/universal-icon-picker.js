...
/*jshint esversion: 8 */
const iconPickerUrl = iconPickerLibUrl.url;
...
(function (root, factory) {
	...
	function UniversalIconPicker(selector, options) {
		...
		this.universalWrap =
			'<div class="uip-modal uip-open" id="uip-modal' +
			this.idSuffix +
			'"><div class="uip-modal--content"><div class="uip-modal--header"><div class="uip-modal--header-logo-area"><span class="uip-modal--header-logo-title">Universal Icon Picker</span></div><div class="uip-modal--header-close-btn"><img src="' +
			(options.closeUrl || iconPickerUrl + '/images/xmark-solid.svg') +
			'" width="20" height="16" alt="Close" title="Close" /></div></div><div class="uip-modal--body"><div id="uip-modal--sidebar' +
			this.idSuffix +
			'" class="uip-modal--sidebar"><div class="uip-modal--sidebar-tabs"></div></div><div id="uip-modal--icon-preview-wrap' +
			this.idSuffix +
			'" class="uip-modal--icon-preview-wrap"><div class="uip-modal--icon-search"><input name="" value="" placeholder="Filter by name..."><img src="' +
			(options.searchUrl ||
				iconPickerUrl + '/images/magnifying-glass-solid.svg') +
			'" width="20" height="16" alt="Search" title="Search" /></div><div class="uip-modal--icon-preview-inner"><div id="uip-modal--icon-preview' +
			this.idSuffix +
			'" class="uip-modal--icon-preview"></div></div></div></div><div class="uip-modal--footer"><button class="uip-insert-icon-button">Insert</button></div></div></div>';

		this.universalDomEle = createDomEle(this.universalWrap);
		this.sidebarTabs = this.universalDomEle.querySelector(
			'.uip-modal--sidebar-tabs'
		);
		this.previewWrap = this.universalDomEle.querySelector(
			'#uip-modal--icon-preview' + this.idSuffix
		);
		this.searchInput = this.universalDomEle.querySelector(
			'.uip-modal--icon-search input'
		);
	...
	UniversalIconPicker.prototype = {
		...
		init() {
			...
			if (this.options.resetSelector) {
				document
					.querySelector(this.options.resetSelector)
					.addEventListener('click', this.options.onReset);
			}
		},
		...
		open() {
			this._loadIconLibraries().then(() => {
				...
				if (!document.getElementById('uip-modal' + this.idSuffix)) {
					...
					this.universalDomEle
						.querySelector('.uip-modal--header-close-btn')
						.addEventListener('click', () => {
					...
					//Insert button
					this.universalDomEle
						.querySelector('.uip-insert-icon-button')
						.addEventListener('click', () => {
							const selected = this.universalDomEle.querySelector(
								'.universal-selected'
							);

							if (selected) {
								const iconHtml =
									selected.querySelector('i').outerHTML;
								const jsonOutput = {
								...
								if (
									!selected
										.querySelector('i')
										.classList.value.match('uip-icon-none')
								) {
									jsonOutput.iconHtml = iconHtml;
									jsonOutput.iconMarkup =
										escapeHtml(iconHtml);
									jsonOutput.iconClass =
										selected.querySelector(
											'i'
										).classList.value;
									jsonOutput.iconText =
										selected.querySelector('i').innerText;
								}
				...
				if (!this.iconEventsLoaded) {
					...
					this.universalDomEle
						.querySelectorAll('.uip-icon-item')
						.forEach((item) => {
				...
				this.universalDomEle
					.querySelector('.uip-modal--icon-search input')
					.focus();
			});
		},
		...
		_iconItemMarkup(libraryName, libraryItem) {
			let markup = '',
				library = libraryItem['icon-style'],
				prefix = libraryItem.prefix;
			if (this.options.allowEmpty) {
				markup +=
					'<div class="uip-icon-item" data-library-id="' +
					library +
					'" data-filter="" data-library-name="' +
					libraryName +
					'"><div class="uip-icon-item-inner"><i class="' +
					prefix +
					' uip-icon-none">&nbsp;</i><div class="uip-icon-item-name" title="None">None</div></div></div>';
			}
			if (prefix.match(/^material-icons/)) {
				libraryItem.icons.forEach(function (item) {
					markup +=
						...
						'">' +
						item +
						'</i><div class="uip-icon-item-name" title="' +
						item +
						'">' +
						item.replace('-', ' ') +
						'</div></div></div>';
		...
		_loadCssFiles() {
			...
			if (
				!loadedDependencies.includes('universal-icon-picker.min.css') &&
				!this.options.loadCustomCss
			) {
				link.rel = 'stylesheet';
				link.type = 'text/css';
				link.href =
					iconPickerUrl +
					'/stylesheets/universal-icon-picker.min.css';
				link.media = 'screen';
			...
			if (this.options.iconLibrariesCss) {
				this.options.iconLibrariesCss.forEach((cssFile) => {
					if (!loadedDependencies.includes(cssFile)) {
						let cssFileLink =
							iconPickerUrl + '/stylesheets/' + cssFile;
		...
		async _loadIconLibraries(i = 0) {
			...
			await fetch(iconLibUrl)
				...
				.then((data) => {
					// Success!
					const camelCasedIconLibrary = iconLib
					...
					if (i + 1 === this.options.iconLibraries.length) {
						...
						this.activeLibraryId =
							this.sideBarList[0]['library-id'];

						//sidebar list markup push
						this.sidebarTabs.innerHTML = this._sideBarListMarkup(
							this.sideBarList
						);
						...
						this.iconWrap =
							this.previewWrap.querySelectorAll('.uip-icon-item');

						//set event lisner to search input
						this.searchInput.addEventListener(
							'keyup',
							debounce(this._searchFunc, 100).bind(this),
							false
						);

						//get all sidebar list item wrapper dom element
						this.sideBarBtn = this.sidebarTabs.querySelectorAll(
						...
						this.sideBarBtn.forEach((item) => {
							item.addEventListener(
								'click',
								this._clickHandlerFunc.bind(this),
								false
							);
						});
		...
		_searchFilterFunc(filterText, dataName) {
			this.filterIcon = Object.entries(this.iconWrap).filter((item) => {
				if (
					-1 === item[1].dataset[dataName].indexOf(filterText) ||
					(this.activeLibraryId !== 'all' &&
						item[1].dataset.libraryId !== this.activeLibraryId)
				) {
					return false;
				}
				return true;
		...
		_setIconAndSidebarList() {
			for (const [libraryName, libraryContent] of Object.entries(
				this.iconLibraries
			)) {
				this._setSideBarList(
					libraryContent.verboseName || getLibraryName(libraryName),
					libraryContent
				);
		...
		_setIconMarkup(libraryName, libraryContent) {
			if (libraryContent.icons !== undefined) {
				this.iconMarkup += this._iconItemMarkup(
					libraryName,
					libraryContent
				);
			} else {
		...
		_sidebarFilterFunc(filterText) {
			this.activeLibraryId = filterText;
			this.filterIcon = Object.entries(this.iconWrap).filter(
				function (item) {
					if (
						'all' === filterText ||
						filterText === item[1].dataset.libraryId
					) {
						return true;
					}
					return false;
		...
		_setSideBarList(libraryName, libraryContent) {
			let listItem;
			if (libraryContent.icons !== undefined) {
				listItem = {
					title: libraryName,
					prefix:
						libraryContent.prefix !== undefined
							? libraryContent.prefix
							: '',
					'list-icon':
						libraryContent['list-icon'] !== undefined
							? libraryContent['list-icon']
							: '',
					'library-id':
						libraryContent['icon-style'] !== undefined
							? libraryContent['icon-style']
							: 'all',
				};
				...
				Object.entries(libraryContent).forEach((item) => {
					listItem = {
						...
						prefix:
							item[1].prefix !== undefined ? item[1].prefix : '',
						'list-icon':
							item[1]['list-icon'] !== undefined
								? item[1]['list-icon']
								: '',
						'library-id':
							item[1]['icon-style'] !== undefined
								? item[1]['icon-style']
								: 'all',
					};
					this.sideBarList.push(listItem);
		...
		_sideBarListMarkup(sideBarList) {
			let markup = '';
			sideBarList.forEach((item) => {
				...
				if ('all' !== item['library-id']) {
					...
					markup +=
						'<div class="uip-modal--sidebar-tab-item' +
						activeClazz +
						'" data-library-id="' +
						item['library-id'] +
						'">' +
					...
					markup +=
						...
						item['library-id'] +
						'"><img src="' +
						(this.options.starUrl ||
							iconPickerUrl + '/images/star-of-life-solid.svg') +
						'" width="13.125px" height="auto" alt="All" title="All" />' +
						item.title.substring(item.title.lastIndexOf('/') + 1) +
						'</div>';
				}
			});
