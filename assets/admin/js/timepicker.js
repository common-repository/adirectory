...
(function ($) {
	...
	const AM_PM = ['AM','PM'], MAX_HOUR = 12, MAX_MIN = 59,
	...
	const DTimePicker = function(elem, options){
		const that = this;
		this.input = $(elem);
		...
		if(options.time){
			let h = parseInt(options.time.substr(0, 2)), m = parseInt(options.time.substr(3, 2));
	...
	DTimePicker.prototype = {
		...
		spinUp () { this.adjustTime('p'); },

		spinDown () { this.adjustTime('m'); },
		...
		setValue () {
			let ap = this.ampm.val(), h = this.hour.val(), m = this.min.val();
			if(ap=="PM") h=parseInt(h)+12;
			const val = h+":"+m+":00.000";
